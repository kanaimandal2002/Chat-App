...
// ✅ Fixed emojiMap keys (removed unnecessary backslashes)
const emojiMap = {
  ':smile:': '😄',
  ':heart:': '❤️',
  ':thumbsup:': '👍',
...
if (message.startsWith('/unban ')) {
  ...
  const emojiText = replaceEmojis(message.replace('/emoji', '').trim());
  broadcast(`${emojiText}`, client.room);
}
...
const clientIndex = clients.findIndex(c => c.socket === socket);
if (clientIndex !== -1) {
const clientNameToRemove = clients[clientIndex].name;
clients.splice(clientIndex, 1);
broadcast(`${clientNameToRemove} has left the chat.\n`, socket);
sendClientCount();
logMessage(`${clientNameToRemove} left the chat.`);
}
});
